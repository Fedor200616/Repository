    ...
    while (!k) {
        ...
        if (c_secname[0] != '\0') {
            ...
            if (!(check_first_char(secname[0]) || RU_check_first_char(secname[0]) && k == 1)) {
                ...
                if (check_char(secname[0]) || RU_check_char(secname[0])) {
                    b_secname = secname;
                    b_secname[0] -= Aa;
...
void instruction() {
    std::ifstream file("instruction.md"); // пробуем текущую папку exe
    if (!file) {
        file.open("../../instruction.md"); // пробуем корень проекта
        if (!file) {
            std::cout << "Ошибка: файл instruction.md не найден!\n";
            _getch();
            return;
        }
    }
    ...
    while (std::getline(file, line)) {
        ...
        while (pos < line.length()) {
            std::cout << line.substr(pos, width) << "\n";
            pos += width;
        }
    }
